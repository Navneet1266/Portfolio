...
export default function Sidebar({ open, onClose, activeId }) {
  ...
  return (
    <div className={`fixed inset-y-0 left-0 z-40 md:w-24 ${open ? '' : 'pointer-events-none md:pointer-events-auto'}`}>
      ...
      <aside role="navigation" aria-label="Sidebar" aria-hidden={!open} className={`absolute md:static top-0 left-0 h-full w-full md:w-24 bg-surface/95 backdrop-blur border-r border-white/10 transition-transform ${open ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        <div className="flex items-center justify-between md:justify-center h-16 px-4 border-b border-white/10">
          ...
          <button className="md:hidden p-3 rounded-xl hover:bg-white/10 touch-manipulation" onClick={onClose}><X/></button>
        </div>
        <ul className="p-4 flex md:flex-col gap-3 md:gap-4">
          {items.map(({id,label,icon:Icon})=>{ const active = activeId===id; return (
            <li key={id}>
              <a href={`#${id}`} onClick={(e)=>handleClick(e,id)} aria-current={active ? 'page' : undefined} className={`group flex items-center md:flex-col gap-3 p-4 md:p-3 rounded-xl border border-white/5 transition ${active ? 'text-glow shadow-glow' : 'hover:bg-white/5 hover:border-white/10 hover:text-accent'}`}>
